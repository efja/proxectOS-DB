...
export class TypeAppController {
  ...
  public create = async (
    ...
    try {
      ...
      if (this.hasMinimumAttributes(type)) {
        data = await this.typeService.create(type);
      }
      ...
      if (
        ...
        data = undefined;
        error = req.t('ERROR.ALREADY_EXIST', { entity: req.t(`${this.TRANSLATION_NAME_MODEL}.NAME`), id: type.id });
      } else {
        data = undefined;
  ...
  public createList = async (
    ...
    try {
      ...
      if (continueProcess && types && types.length > 0) {
        data = await this.typeService.createList(types);
      }
  ...
  public getAll = async (
    ...
    try {
      ...
      let data = await this.typeService.getAll(queryParams.getQueryObj(), orderBy, limit, offset);
  ...
  public get = async (
    ...
    try {
      ...
      let data = await this.typeService.get(id, queryParams.getQueryObj());
  ...
  public update = async (
    ...
    try {
      ...
      if (this.hasMinimumAttributes(type)) {
        data = await this.typeService.update(id, type);
      }
   ...
   public modify = async (
    ...
    try {
      ...
      if (objPatch.length > 0) {
        ...
        data = await this.typeService.modify(id, objPatch);
  ...
  public delete = async (
    ...
    try {
      ...
      let data = await this.typeService.delete(id);
