...
export class RequirementController {
  ...
  // ** ATRIBUTOS
  // ************************************************************************************************
  private TRANSLATION_NAME_MODEL : string = 'REQUIREMENT';
  public requirementService : RequirementService = new RequirementService();
  ...
  public create = async (
    ...
    try {
      ...
      const requirement: Requirement = new Requirement(req.body);

      let data;

      if (this.hasMinimumAttributes(requirement)) {
        data = await this.requirementService.create(requirement);
      }
      ...
      if (
        ...
        data = undefined;
        error = req.t('ERROR.ALREADY_EXIST', { entity: req.t(`${this.TRANSLATION_NAME_MODEL}.NAME`), id: requirement.id });
      } else {
        data = undefined;
  ...
  public createList = async (
    ...
    try {
      ...
      const requirements: Requirement[] = req.body;
      ...
      for (let i = 0; i < requirements.length; i++) {
        ...
        if (this.hasMinimumAttributes(item)) {
          ...
          requirements[i] = new Requirement(item);
        } else {
      ...
      if (continueProcess && requirements && requirements.length > 0) {
        data = await this.requirementService.createList(requirements);
      }
  ...
  public getAll = async (
    ...
    try {
      ...
      let data = await this.requirementService.getAll(queryParams.getQueryObj(), orderBy, limit, offset);
  ...
  public get = async (
    ...
    try {
      ...
      let data = await this.requirementService.get(id, queryParams.getQueryObj());
  ...
  public update = async (
    ...
    try {
      ...
      if (this.hasMinimumAttributes(requirement)) {
        data = await this.requirementService.update(id, requirement);
      }
   ...
   public modify = async (
    ...
    try {
      ...
      if (objPatch.length > 0) {
        ...
        data = await this.requirementService.modify(id, objPatch);
  ...
  public delete = async (
    ...
    try {
      ...
      let data = await this.requirementService.delete(id);
  ...
  // ************************************************************************************************
  /**
   * Comproba se o Requirement pasado ten os atributos mínimos que o modelo necesita.
   *
   * @param item Requirement que se vai a avaliar
   * @returns Boolean
   */
  private hasMinimumAttributes = (item: Requirement): Boolean => {
    let result = false;
