...
export class RoleService {
  ...
  private async createDbConnection() {
    ...
    await this.db.init();
    this.respository = this.db.getRepository(Role);
  }
  ...
  // -- POST
  // ------------------------------------------------------------------------------------------------
  public async create(role: Role): Promise<any> {
    ...
    try {
      ...
      if (role.id != null) {
        temp = await this.respository.findOne(role.id);
      }

      if (temp == null) {
        temp = new Role();
        temp.assign(role, { em: this.db.orm.em });
  ...
  public async createList(roles: Role[]): Promise<any> {
    let result : any = HttpStatus.CONFLICT;
    ...
    try {
      ...
      if (roleIds != null) {
        temp = await this.respository.find(roleIds);
      }

      if (temp == null || temp.length == 0) {
        await this.respository.persistAndFlush(roles);
        result = roles;
      }
    } catch (error) {
      result = null;
  ...
  // -- PUT
  // ------------------------------------------------------------------------------------------------
  public async update(id: string, role: Role): Promise<any> {
    let result : any = HttpStatus.NOT_FOUND;
    ...
    try {
      ...
      if (temp != null) {
        try {
          let updateData = await getEntityForUpdate(role, 'Role');
