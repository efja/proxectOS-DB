...
export class UserService {
  ...
  private async createDbConnection() {
    ...
    await this.db.init();
    this.respository = this.db.getRepository(User);
  }
  ...
  public async create(user: User): Promise<any> {
    ...
    try {
      ...
      if (user.id != null) {
        temp = await this.respository.findOne(user.id);
      }

      if (temp == null) {
        temp = new User();
        temp.assign(user, { em: this.db.orm.em });
  ...
  public async createList(users: User[]): Promise<any> {
    let result : any = HttpStatus.CONFLICT;
    ...
    try {
      ...
      if (userIds != null) {
        temp = await this.respository.find(userIds);
      }

      if (temp == null || temp.length == 0) {
        await this.respository.persistAndFlush(users);
        result = users;
      }
    } catch (error) {
      result = null;
  ...
  // -- PUT
  // ------------------------------------------------------------------------------------------------
  public async update(id: string, user: User): Promise<any> {
    let result : any = HttpStatus.NOT_FOUND;
    ...
    try {
      ...
      if (temp != null) {
        try {
          let updateData = await getEntityForUpdate(user, 'User');
