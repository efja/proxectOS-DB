...
describe('1: Probas DATOS API - Resources (PATCH)', () => {
    ...
    test(`1.1: Actualizar Resource: <${dataList.resources[0].id}>`, async() => {
        ...
        const objPatch = ooPatch.diff(resource0, resource1);
        ...
        expect(message).toBe(i18next.t('SUCCESS.UPDATE', { entity: i18next.t('RESOURCE.NAME'), id: resource1.id }));
    });
...
describe('2: Probas DATOS API - Resources ERROS (PATCH)', () => {
    ...
    test(`2.2: Actualizar Resource que non existe:`, async() => {
        ...
        do {
            resource0._id = new ObjectId();
        } while (resource0._id == dataList.resources[0]._id);
