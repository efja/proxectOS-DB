...
describe('1: Probas DATOS API - UserGroups (PUT)', () => {
    ...
    test(`1.1: Actualizar UserGroup: <${dataList.userGroups[0].id}>`, async() => {
        ...
        const response = await request.put(`${API_BASE}/${ENDPOINT}/${dataList.userGroups[0].id}`).send(userGroup1);
        const {
            code,
            data,
            message,
...
describe('1: Probas DATOS API - UserGroups ERROS (PUT)', () => {
    // ************************************************************************************************
    // ** ATRIBUTOS
    // ************************************************************************************************
    const ENDPOINT = "userGroups";
    ...
    test(`2.1: Actualizar UserGroup con datos erróneos:`, async() => {
        ...
        userGroup1.createdAt = userGroup0.name + FAKE_TEXT; // Dato erróneo

        const response = await request.put(`${API_BASE}/${ENDPOINT}/${userGroup0.id}`).send(userGroup1);
        const {
            code,
            data,
            message,
    ...
    test(`2.2: Actualizar UserGroup que non existe:`, async() => {
        ...
        expect(error).toBe(i18next.t('ERROR.NOT_FOUND_MALE', { entity: i18next.t('USER_GROUP.NAME'), id: userGroup0.id }));
    });
});
