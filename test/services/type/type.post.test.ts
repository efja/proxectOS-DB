...
describe('1: Probas DATOS API - Types (POST)', () => {
    ...
    test(`1.1: Crear Type: <${dataList.types[0].id}>`, async() => {
        ...
        expect(message).toBe(i18next.t('SUCCESS.CREATE', { entity: i18next.t('TYPE.NAME') }));
    });
    ...
    test('1.2: Crear lista de Types:', async() => {
        ...
        expect(message).toBe(i18next.t('SUCCESS.CREATE_LIST', { entity: i18next.t('TYPE.NAME_PLURAL') }));
    });
});
...
describe('2: Probas DATOS API - Types ERROS (POST)', () => {
    ...
    test(`2.2: Crear Type: <${dataList.types[0].id}> QUE XA EXISTE`, async() => {
        ...
        expect(error).toBe(i18next.t('ERROR.ALREADY_EXIST', { entity: i18next.t('TYPE.NAME'), id: type.id }));
    });
    ...
    test('2.3: Crear lista de Types algúns con datos erróneos:', async() => {
        ...
        expect(error).toBe(i18next.t('ERROR.CREATE_LIST', { entity: i18next.t('TYPE.NAME_PLURAL') }));
    });
});