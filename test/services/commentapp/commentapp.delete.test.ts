...
describe('1: Probas DATOS API - CommentApps (DELETE)', () => {
    ...
    test(`1.1: Borrar CommentApp: <${dataList.comments[0].id}>`, async() => {
        ...
        expect(errorGet).toBe(i18next.t('ERROR.NOT_FOUND', { entity: i18next.t('COMMENT.NAME'), id: dataList.comments[0].id }));
    });
});
...
describe('2: Probas DATOS API - CommentApps ERROS (DELETE)', () => {
    ...
    test(`2.1: Borrar CommentApp inexistente:`, async() => {
        ...
        expect(error).toBe(i18next.t('ERROR.DELETE', { entity: i18next.t('COMMENT.NAME'), id: `${dataList.comments[0].id}${FAKE_TEXT}` }));
    });
});
